...
<x-modal id="editAdmissionModal{{ $admission->id }}" title="Edit Admission" :showButton="false">
    <form method="POST" action="{{ route('admissions.update', $admission->id) }}" enctype="multipart/form-data">
        ...
        <div class="grid gap-4 mb-4">
            ...
            <div>
                <label for="edit_image_{{ $admission->id }}" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Admission Image</label>
                <input type="file" name="image" id="edit_image_{{ $admission->id }}"
        ...
        <div class="flex justify-end space-x-3">
            <button type="button"
                    data-modal-hide="editAdmissionModal{{ $admission->id }}"
