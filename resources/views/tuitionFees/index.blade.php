...
@section('content')
    <div class="max-w-screen-xl mx-auto p-6">
        ...
        <div class="flex justify-between items-center mb-8">
            <div>
                <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Tuition Fees</h1>
            </div>

            <!-- Add Tuition Fee Button -->
            @can('add-admission')
                <button data-modal-target="addtuitionFeeModal" data-modal-toggle="addtuitionFeeModal"
                ...
                <div class="relative group">
                    ...
                    <div class="absolute z-10 invisible opacity-0 group-hover:visible group-hover:opacity-100 transition-all duration-300 top-full left-1/2 transform -translate-x-1/2 mt-2 px-3 py-2 text-xs font-medium text-white bg-gray-900 rounded-lg shadow-sm dark:bg-gray-700">
                        <span class="flex items-center">
                            You don't have permission to add tuition fees
                        </span>
                        ...
                        <tr class="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors duration-150">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center space-x-4">
                                    <img src="{{ asset('storage/' . $tuitionFee->image) }}" alt="Tuition Fee Image"
                                        class="w-20 h-20 object-cover rounded-lg shadow-sm border border-gray-200 dark:border-gray-600">
                                    <div class="flex-1 min-w-0">
                                        <p class="text-sm font-medium text-gray-900 dark:text-white truncate">
                                            {{ $tuitionFee->name ?? 'Tuition Fee Image' }}
                                        </p>
                                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                                            Uploaded: {{ $tuitionFee->created_at->format('M j, Y') }}
                                        </p>
                                        <p class="text-xs text-gray-500 dark:text-gray-400">
                                            @php
                                                $filePath = public_path('storage/' . $tuitionFee->image);
                                                $fileSize = file_exists($filePath)
                                                    ? round(filesize($filePath) / 1024)
                                                    : 0;
                                            @endphp
                                            Size: {{ $fileSize }} KB
                                        </p>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex flex-wrap gap-2">
                                    ...
                                    @can('update-admission')
                                        <button data-modal-target="edittuitionFeeModal{{ $tuitionFee->id }}"
                                            data-modal-toggle="edittuitionFeeModal{{ $tuitionFee->id }}"
                                    ...
                                    @else
                                        ...
                                        <div class="relative group">
                                            ...
                                            <div class="absolute z-10 invisible opacity-0 group-hover:visible group-hover:opacity-100 transition-all duration-300 bottom-full left-0 mb-2 px-3 py-2 text-xs font-medium text-white bg-gray-900 rounded-lg shadow-sm dark:bg-gray-700 w-48">
                                                <span class="flex items-center">
                                                    You don't have permission to edit tuition fees
                                                </span>
                                    ...
                                    @can('delete-admission')
                                        <form action="{{ route('tuitionFees.destroy', $tuitionFee->id) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                                                onclick="return confirm('Are you sure you want to delete this tuition fee?')"
                                    ...
                                    @else
                                        ...
                                        <div class="relative group">
                                            ...
                                            <div class="absolute z-10 invisible opacity-0 group-hover:visible group-hover:opacity-100 transition-all duration-300 bottom-full left-0 mb-2 px-3 py-2 text-xs font-medium text-white bg-gray-900 rounded-lg shadow-sm dark:bg-gray-700 w-52">
                                                <span class="flex items-center">
                                                    You don't have permission to delete tuition fees
                                                </span>
                                                <div class="absolute top-full left-4 transform border-4 border-transparent border-t-gray-900 dark:border-t-gray-700"></div>
                                            </div>
                                        </div>
                                    ...
                                    <a href="{{ asset('storage/' . $tuitionFee->image) }}" target="_blank"
        ...
        @else
            ...
            <div class="text-center py-16 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
                <div class="max-w-md mx-auto">
                    ...
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">No tuition fee yet</h3>
                    <p class="text-gray-500 dark:text-gray-400 mb-6">Get started by adding tuition fee.</p>
                </div>
            </div>
        @endif
    </div>
    ...
    @can('add-admission')
        @include('tuitionFees.create')
    @endcan

    @can('update-admission')
        @if (isset($tuitionFees) && $tuitionFees->count())
            @include('tuitionFees.edit')
        @endif
    @endcan
